...
# Read an ASIN list from config and turn it into a clean set we can reliably compare against.
def _config_asin_set(config: dict[str, Any], key: str) -> set[str]:
    """Normalize config ASIN list entries to uppercase 10-char ASINs."""
    raw = config.get(key)
...
# Apply required keywords and ASIN blacklist rules and return a human-readable drop list for logs.
def _apply_required_keywords_and_yaml_blacklist(
    candidates: list[dict[str, Any]],
    config: dict[str, Any],
    yaml_blacklist: set[str],
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    """YAML blacklist ASINs drop first; required_keywords and title blacklist apply to all.
...
def run_search_filter_pipeline(
    ...
    *,
    ...
    """Stage1 gates + YAML blacklist + required_keywords -> state-engine rows."""
    ...
    stage1, bl_kw_drops = _apply_required_keywords_and_yaml_blacklist(stage1_core, config, blacklist_set)
