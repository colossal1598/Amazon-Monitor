  ...
  apps: [
    {
      ...
      // continuous scraping. The streaming engine already recycles its browser every
      // browser_recycle_minutes, so this only resets the Python process itself, not
      // scrape state (SQLite-backed).
      cron_restart: "0 5 * * *",
      env: {
        PYTHONUNBUFFERED: "1",
        ...dotEnv,
